...
async function debugImageKit() {
  ...
  try {
    ...
    const files = await imagekit.listFiles({
      path: 'ebs-closet/products/'
    });
    console.log('Files in ebs-closet/products:', files);
  } catch (error) {
  ...
  try {
    ...
    const fileDetails = await imagekit.getFileDetails('ebs-closet/products/product_1755422634159_worldisyours.jpg');
  ...
  try {
    ...
    const result = await uploadImage(mockFile, 'ebs-closet/products', 'debug');
