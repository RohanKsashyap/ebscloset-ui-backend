  ...
  try {
    ...
    const categoriesWithCount = await Promise.all(categories.map(async (cat) => {
      const productCount = await Product.countDocuments({ 
        $or: [
          { categoryId: cat._id },
          { category: cat.name } // Support legacy string category
        ]
      });
...
router.post('/admin/categories', async (req, res) => {
  try {
    ...
    if (req.files && req.files.image) {
      ...
      const uploadResponse = await uploadImage(file, 'ebs-closet/categories', 'category');
...
router.post('/admin/images', async (req, res) => {
  try {
    ...
    const file = req.files.image;
    const uploadResponse = await uploadImage(file, 'ebs-closet/gallery', 'gallery');
